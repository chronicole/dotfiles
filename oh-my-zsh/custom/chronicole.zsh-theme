...
# PROMPT=$'%m %n %/[%t]$' ------ which outputs custom prompt settings
# hostname whoami home directory [time]$
# -------------------------------------------------------------------

if [[ "$TERM" != "dumb" ]] && [[ "$DISABLE_LS_COLORS" != "true" ]]; then

    PROMPT='%{$fg[magenta]%}%n%{$reset_color%} at %{$fg[yellow]%}%m%{$reset_color%} in %{$fg_bold[green]%}${PWD/#$HOME/~}%{$reset_color%}$(git_prompt_info)${return_code}$(git_prompt_status)%{$reset_color%} %{$fg_bold[yellow]%}➜ %{$reset_color%} '
